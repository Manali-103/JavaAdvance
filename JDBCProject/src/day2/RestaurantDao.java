...
@Override
	...
	public Restaurant getOne(Integer id) {
		...
		try(
			...
			String sqlQuery = 
					"select restaurantId, name, cuisine, branchCount from Restaurant where Restaurant = ?";
	...
	public void create(Restaurant newRestaurant) {
		String sqlQuery =
		"insert into Restaurant(restaurantId, name,cuisine,branchCount) values(?,?,?,?)";
		try(
				Connection conn = JdbcUtils.buildConnection();
				PreparedStatement pstmt = conn.prepareStatement(sqlQuery);
				){
	...
	public void deleteOne(Integer id) {
		String sqlQuery = "delete from Restaurant where restaurantId = ?";
		try(
				Connection conn = JdbcUtils.buildConnection();
				PreparedStatement pstmt = conn.prepareStatement(sqlQuery);
				){
	...
	public void update(Restaurant modifiedRestaurant) {
		String sqlQuery = "update Restaurant set name = ?, cuisine = ?, branchCount = ? where restaurantId = ?";
		try(
				Connection conn = JdbcUtils.buildConnection();
				PreparedStatement pstmt = conn.prepareStatement(sqlQuery);
				){
